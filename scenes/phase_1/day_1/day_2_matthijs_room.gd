...
func _matthijs_room_day_2_aftermath_good(): 
	...
	await _play_bubble(child1_bubble, "child1", "Vader told me inlanders have no manners!", false)
	...
	await _play_bubble(child1_bubble, "child1", "Tjee, there’s no need to be so angry.", false)
	await _play_bubble(child1_bubble, "child2", "You’re boring, Matthijs!", false)

	await _play_bubble(player_bubble, "mc", "Bangebroek. You said that yet you keep on playing with my toy.", true)
	...
	await _play_bubble(moeder_bubble, "moeder", "Schatje, did you have fun playing with your friends?", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Schatje...", false)
	await _play_bubble(moeder_bubble, "moeder", "It’s not bad if it’s the truth, ja?.", false)
	await _play_bubble(moeder_bubble, "moeder", "Our schatje is so smart.", false)
	await _play_bubble(moeder_bubble, "moeder", "But Mamma doesn't want you to think of such trivial matters.", false)
	await _play_bubble(moeder_bubble, "moeder", "You should focus on your study and have fun, ja?", false)
...
func _matthijs_room_day_2_aftermath_neutral():
	...
	await _play_bubble(moeder_bubble, "moeder", "Schatje, did something happen?", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Nou, afternoon tea would be ready in a moment.", false)
	await _play_bubble(moeder_bubble, "moeder", "Do put those toys back inside the box later, ja?", false)
	...
	await _play_bubble(moeder_bubble, "moeder", " Schatje? What happened?", false)
...
func _matthijs_room_day_2_aftermath_bad():
	...
	await _play_bubble(child2_bubble, "child2", "Ah, vader told me inlanders can make cars from orange peels too!", false)
	shake_strength = randomStrength
	...
	await _play_bubble(child1_bubble, "child1", "Look at him crying! Huilie-balkie!", false)
	moeder.show()
	await _walk_sprite_to_player(moeder, 20.0)
	await _play_bubble(moeder_bubble, "moeder", "Schatje, did something happen?", false)
	await _play_bubble(player_bubble, "mc", "...", false)
	...
	await _play_bubble(player_bubble, "mc", "They’re not helping… bangebroek! All of them", true)
	...
	await InterludeManager.interlude_finished
	await _walk_sprite_to_player(moeder, 20.0)
	await _play_bubble(moeder_bubble, "moeder", "Schatje? Mamma wants to talk", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "..Schatje, Mamma is disappointed in you.", false)
	await _play_bubble(moeder_bubble, "moeder", "Mamma wouldn’t do anything to our Schatje.", false)
	await _play_bubble(player_bubble, "mc", "...Matthijs is sorry.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Natuurlijk! Mamma would still love you.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Matthijs should tell Mamma immediately, ja?", false)
	await _play_bubble(player_bubble, "mc", "Yes, Mamma.", false)
