...
func _matthijs_corridor_day_3_aftermath_good(): 
	...
	await _play_bubble(moeder_bubble, "moeder", "Is the kaasstengels not to your liking, Schatje?", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Natuurlijk! Anything!", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Yes, Mamma is fine, Schatje.", false)
	...
	await _play_bubble(player_bubble, "mc", "My friends were also talking about a new play in the schouwburg.", false)
	await _play_bubble(moeder_bubble, "moeder", "Our sweet Schatje… (soft laugh)", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Finish your tea and eat more kaasstengels, ja?", false)
...
func _matthijs_corridor_day_3_aftermath_neutral():
	...
	await _play_bubble(njai1_bubble, "njai1", "Jongeheer? Our apologies!", false)
	await _play_bubble(njai1_bubble, "njai1", "Truly, truly we meant nothing bad. Mevrouw-", false)
	...
	await _play_bubble(njai2_bubble, "njai2", "Jongeheer, Mevrouw was with child when you were much younger.", false)
	await _play_bubble(njai2_bubble, "njai2", "Back then, Mevrouw always went with Mijnheer everywhere. But-", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Explain what, Schatje?", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "You shouldn’t hide something from Mamma, Schatje.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "It seems like our Schatje is pretty stubborn today.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Be a good boy for Mamma and wait for mama in the dining table, ja?", false)
	await _play_bubble(player_bubble, "mc", "Yes, Mamma.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Schatje, today’s weather seemed suitable for a walk, ja?", false)
...
func _matthijs_corridor_day_3_aftermath_bad():
	...
	await _play_bubble(njai2_bubble, "njai2", "Jongeheer, please calm down. We-", false)
	...
	tween.tween_property(moeder, "modulate:a", 1.0, 0.5)
	await tween.finished
	await _play_bubble(moeder_bubble, "moeder", "Schatje, what is happening?", false)
	await _play_bubble(moeder_bubble, "moeder", "...", false)
	...
	await _play_bubble(moeder_bubble, "moeder", "Schatje, listen to Mamma carefully.", false)
	await _play_bubble(moeder_bubble, "moeder", "Yes, they did wrong. But you shouldn’t have broken Mamma’s vase, or anything when you’re mad, ja?", false)
	await _play_bubble(player_bubble, "mc", "Yes, Mamma.. Matthijs is sorry..", false)
	await _play_bubble(moeder_bubble, "moeder", "There’s no need to worry, Schatje.", false)
	...
	await _play_bubble(moeder_bubble, "moeder", " Start eating your kaasstengels, ja?", false)
...
func matthijs_corridor_day3_after_premise():
	...
	await _play_bubble(njai1_bubble, "njai1", "Mbok, why does Mijnheer and Mevrouw pamper Young Master so much?", false)
	await _play_bubble(njai2_bubble, "njai2", "You didn’t know?", false)
	await _play_bubble(njai2_bubble, "njai2", "Mevrouw had a miscarriage a couple years back.", false)
	...
	await _play_bubble(njai1_bubble, "njai1", "So Jongeheer should’ve had a sibling?", false)
	await _play_bubble(njai1_bubble, "njai1", "Is that why Mevrouw  doesn’t follow Mijnheer to work?", false)
